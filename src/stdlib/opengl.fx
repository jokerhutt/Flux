...
namespace OpenGL
{
    ...
    def gl_load_extensions() -> void
    {
        ...
        _glUniformMat4fv        = wglGetProcAddress("glUniformMat4fv\0");
    ...
    // Dot product
    def glvec3_dot(GLVec3* a, GLVec3* b) -> float
    {
        return a.x * b.x + a.y * b.y + a.z * b.z;
    };

    // Cross product: out = a x b
    def glvec3_cross(GLVec3* a, GLVec3* b, GLVec3* out) -> void
    {
    ...
    // Length of a vector
    def glvec3_length(GLVec3* v) -> float
    {
        return sqrt(v.x * v.x + v.y * v.y + v.z * v.z);
    };

    // Normalize a vector in-place (no-op if near-zero length)
    def glvec3_normalize(GLVec3* v) -> void
    {
        float len = glvec3_length(v);
    ...
    // Subtract: out = a - b
    def glvec3_sub(GLVec3* a, GLVec3* b, GLVec3* out) -> void
    {
        out.x = a.x - b.x;
        out.y = a.y - b.y;
        out.z = a.z - b.z;
    ...
    // Add: out = a + b
    def glvec3_add(GLVec3* a, GLVec3* b, GLVec3* out) -> void
    {
        out.x = a.x + b.x;
        out.y = a.y + b.y;
        out.z = a.z + b.z;
    ...
    // Scale: out = v * s
    def glvec3_scale(GLVec3* v, float s, GLVec3* out) -> void
    {
        out.x = v.x * s;
        out.y = v.y * s;
        out.z = v.z * s;
    ...
    // OpenGL Mat4
    // ===
    def glmat4_lookat(GLVec3* eye, GLVec3* target, GLVec3* up, Mat4* out) -> void
    {
        GLVec3 f,s,u;
        f.x = target.x - eye.x;
        f.y = target.y - eye.y;
        ...
        out.m30 = -glvec3_dot(@s, eye);   // was 0.0 - dot
        out.m31 = -glvec3_dot(@u, eye);
        out.m32 =  glvec3_dot(@f, eye);
        out.m33 = 1.0;

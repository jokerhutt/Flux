...
// J/L = shift Julia constant real part
// I/K = shift Julia constant imaginary part
// ============================================================================
...
// Compute Julia set iteration count for z0 = (x0, y0), constant c = (cr, ci)
def julia(u64 x0, u64 y0, u64 cr, u64 ci, int max_iter) -> int
{
    u64 x, y, xx, yy, xtemp;
    float xxhi, yyhi;
    int iter;
    ...
    while (iter < max_iter)
    {
        xx    = dd_mul(x, x);
        yy    = dd_mul(y, y);
        // Use hi parts only for the magnitude bailout check
        xxhi  = dd_hi(xx);
        yyhi  = dd_hi(yy);
        if (xxhi + yyhi > 4.0) { return iter; };
        // z = z^2 + c:  x' = x^2 - y^2 + cr,  y' = 2*x*y + ci
        xtemp = dd_add(dd_sub(xx, yy), cr);
        y     = dd_add(dd_scale(dd_mul(x, y), 2.0), ci);
        x     = xtemp;
        iter++;
    };
...
def iter_to_color(int iter, int max_iter, float* r, float* g, float* b) -> void
{
    ...
    // First half: violet -> magenta (r rises, g=0, b stays 1)
    // Second half: magenta -> gold (r stays 1, g rises, b falls)
    if (t < 0.5)
    {
        s = t * 2.0;
        *r = s;
...
def main() -> int
{
    Window win( "Julia Set - W/S: Zoom  A/D: Pan X  Up/Down: Pan Y  J/L: Re(c)  I/K: Im(c)\0", 100, 100, WIN_W, WIN_H);
    ...
    // Julia constant c = cr + ci*i  (classic interesting default: -0.7 + 0.27015i)
    u64 cr, ci;
    cr = dd_pack(-0.7,     0.0);
    ci = dd_pack( 0.27015, 0.0);
    ...
    while (win.process_messages())
    {
        ...
        // Shift Julia constant real part
        if ((j_state `& 0x8000) != 0)
        {
        ...
        // Shift Julia constant imaginary part
        if ((i_state `& 0x8000) != 0)
        {
        ...
        // Render Julia set as colored quads - all complex math in double-double
        ...
        while (row < rows)
        {
            ...
            while (col < cols)
            {
                ...
                iter = julia(fx, fy, cr, ci, dyn_max_iter);
